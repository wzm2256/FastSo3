...
# so2_conv = so2.SO2_conv(c_in, c_out, L, c_L0_in=c_L0_in, c_L0_out=c_L0_out, bias=True)
#### test SO2_conv_e
# w = torch.randn((b, c_hidden, 2 * L))
# w_L0 = torch.randn((b, c_hidden))
# so2_conv = so2.SO2_conv_e(c_in, c_out, c_hidden, L, c_L0_in=c_L0_in, c_L0_out=c_L0_out, bias=True)
#### test SO2_conv_c
# w = torch.randn((b, c_in, L, L * 2))
# w_L0 = torch.randn((b, c_L0_in + c_in * L))
# so2_conv = so2.SO2_conv_c(c_in, L, c_L0_in=c_L0_in, bias=True)
#### test SO2_mix
so2_conv = so2.SO2_mix_c(c_in, c_out, L, c_L0_in=c_L0_in, c_L0_out=c_L0_out, bias=True)
...
###### test mix
message, message_0 = so2.get_message_mix(x, x_L0, edges_vec, so2_conv, L)
...
# a forwad pass on the rotated graph
message_R, message_0R = so2.get_message_mix(x_R, x_L0,edges_vec_R, so2_conv, L)
